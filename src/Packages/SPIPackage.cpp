...
SPIPackage::SPIPackage() : MaxBotPackageBase() {
    ...
    auto frontDownIR = std::make_unique<Sensor::GP2Y0A21YK>(spi, 0);
    auto rearIR = std::make_unique<Sensor::GP2Y0A21YK>(spi, 2);
    auto frontIR = std::make_unique<Sensor::GP2Y0A710K>(spi, 1);
    auto frontLeftIR = std::make_unique<Sensor::GP2Y0A02YK>(spi,4 );
    auto frontRightIR = std::make_unique<Sensor::GP2Y0A02YK>(spi, 5);
